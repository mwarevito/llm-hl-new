...
class MarketDataCollector:
    ...
    def get_market_snapshot(self, symbol: str = "BTC") -> Dict:
        ...
        # Hyperliquid returns candles with keys: t, o, h, l, c, v, n
        ...
        for col in required_cols:
            if col not in df.columns:
                logger.warning(f"Missing column {col} in candle data")
                df[col] = 0
        ...
        # Handle both list and dict formats for orderbook levels
        try:
            # Check if levels are in dict format (e.g. {'px': ..., 'sz': ...})
            is_dict_format = False
            if book['levels'][0] and isinstance(book['levels'][0][0], dict):
                is_dict_format = True

            if is_dict_format:
                best_bid = float(book['levels'][0][0]['px']) if book['levels'][0] else 0
                best_ask = float(book['levels'][1][0]['px']) if book['levels'][1] else 0
                bid_depth_10 = sum([float(b['sz']) for b in book['levels'][0][:10]])
                ask_depth_10 = sum([float(a['sz']) for a in book['levels'][1][:10]])
            else:
                # Assume list format [price, size]
...
class LLMTradingDecision:
    ...
    def __init__(self, config: Optional[LLMConfig] = None):
        ...
        logger.info(f"LLM initialized: {self.provider}/{self.model} (temp={self.config.temperature})")
...
class LLMTradingAgent:
    ...
    def __init__(self,
        ...
        self.llm_decision = LLMTradingDecision()
